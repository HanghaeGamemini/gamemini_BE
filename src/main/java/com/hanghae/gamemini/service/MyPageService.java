...
public class MyPageService {
    ...
    public  ResponseEntity<PrivateResponseBody> getMyPost(User user){
       List<Post> posts = postRepository.findAllByUsername(user);
        List<PostResponseDto> postResponseDtos = new ArrayList<>();

        if(posts.isEmpty()){
            return new ResponseEntity<>(new PrivateResponseBody(CommonStatusCode.NO_ARTICLE),HttpStatus.OK);
        }
        for(Post post :posts){
    ...
    public ResponseEntity<PrivateResponseBody> getMyComment(User user){
        List<Comment> comments = commentRepository.findAllByUsername(user);
    ...
    public ResponseEntity<PrivateResponseBody> getMyLikePost(User user){
        List<Likes> likesList = likeRepository.findByUsername(user); //좋아요가 있는지 확인
